Sub 一般薬剤師異動時必要書類印刷()
    ...
    If ws.Cells(8, 9).value <> "" Or ws.Cells(5, 9).value <> "" Then
        sheetCount = sheetCount + 1
        ReDim Preserve sheetArray(1 To sheetCount)
...
Sub 厚生局異動届作成()
    ...
    Dim upDate As Date
    Dim orgValue As String, chgValue As String
    Dim orgHours As Variant, chgHours As Variant
...
End Sub
Sub 変更ログ記録()
    Dim strName As String, logNo As Variant
...
Sub 初期化(ws As Worksheet)
    ws.Range("E2").Formula = "=IF($C$4="""",C98,VLOOKUP(検索!$C$4,LOG!$A$2:$Y$1048507,3,0))"
    ws.Range("F2").Formula = "=VLOOKUP(E2,薬剤師マスタ!C:V,17,0)"
    ws.Range("E3:E12").Formula = "=IF($C$4="""",INDIRECT(""C""&ROW($A$151)+ROW()*3),VLOOKUP(検索!$C$4,LOG!$A$1:$Y$1048507,ROW()*2-1,0))"
    ws.Range("F3:F12").Formula = "=IFERROR(IF($C$4="""",IFERROR(INDIRECT(""C""&ROW($A$152)+ROW()*3),""""),IF(E3<>"""",VLOOKUP(検索!$C$4,LOG!$A$1:$Y$1048507,ROW()*2,0),"""")),J3)"
    ws.Range("I2").Formula = "=IF(A9<>"""",A9,E2)"
    'ws.Range("J2").Formula = "=DGET(薬剤師マスタ[#すべて],""週労働時間"",検索!I1:I2)"
    ws.Range("I3:I12").Formula = "=INDIRECT(""C""&ROW($A$151)+ROW()*3)"
    ws.Range("J3:J12").Formula = "=INDIRECT(""C""&ROW($A$152)+ROW()*3)"
End Sub
Sub 厚生局異動届印刷確認()
    Dim rc As VbMsgBoxResult
